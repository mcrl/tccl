...
template <typename T>
ncclResult_t ncclCudaHostCallocDebugWithNuma(T** ptr, size_t nelem, int numaIdx, const char *filefunc, int line) {
  ncclResult_t result = ncclSuccess;
  cudaStreamCaptureMode mode = cudaStreamCaptureModeRelaxed;
  *ptr = nullptr;
  CUDACHECK(cudaThreadExchangeStreamCaptureMode(&mode));

  struct bitmask *oldmask, *newmask;
  if (numaIdx != -1) {
    NCCLCHECK(tcclLimitNuma(numaIdx, &oldmask, &newmask));
  }
  CUDACHECKGOTO(cudaHostAlloc(ptr, nelem*sizeof(T), cudaHostAllocMapped), result, finish);
  if (numaIdx != -1) {
    NCCLCHECK(tcclUnlimitNuma(oldmask, newmask));
    NCCLCHECK(tcclCheckNuma(*ptr, numaIdx));
    INFO(NCCL_ALLOC | NCCL_TCCL, "%s:%d Cuda Host Alloc Size %ld pointer %p NUMA %d", filefunc, line, nelem*sizeof(T), *ptr, numaIdx);
  }
  memset(*ptr, 0, nelem*sizeof(T));
finish:
  CUDACHECK(cudaThreadExchangeStreamCaptureMode(&mode));
...
#define ncclCudaHostCallocWithNuma(...) ncclCudaHostCallocDebugWithNuma(__VA_ARGS__, __FILE__, __LINE__)
