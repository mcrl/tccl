...
#define NCCL_FUNC4(func, devredop, type, specialized) \
  ...
  NCCL_FUNC5(func, TCCL,           devredop, type, specialized)
...
static ncclResult_t addCollToPlan(
  ...
  // nCollChannels is the number of total channels available (i.e., comm->nChannels in normal cases)
  // nBid is the number of channels to use
  struct ncclKernelPlan::Channel *chans = plan->channels;

  // if TCCL enabled, search TCCL channels instead
  int channelBegin = 0, channelEnd = nCollChannels;
  if (comm->tcclComm.enabled) {
    channelBegin = comm->tcclComm.channelBegin;
    channelEnd = comm->tcclComm.channelEnd;
  }
  TRACE(NCCL_TCCL, "nCollChannels %d nBid %d channelBegin %d channelEnd %d", nCollChannels, nBid, channelBegin, channelEnd);
  ...
  least[0] = channelBegin;
  int maxIndexInLeast = 0;
  size_t maxBytesInLeast = chans[channelBegin].collBytes;
  // Initialize least[] such that the first nBid channels are accounted for.
  for (int b=1; b < nBid; b++) {
    least[b] = channelBegin + b;
    if (maxBytesInLeast < chans[channelBegin + b].collBytes) {
      maxIndexInLeast = b;
      maxBytesInLeast = chans[channelBegin + b].collBytes;
    }
  }
  ...
  for (int c=nBid; c < nCollChannels; c++) {
    if (chans[channelBegin + c].collBytes < maxBytesInLeast) {
      least[maxIndexInLeast] = channelBegin + c;
  ...
  for (int bid=0; bid < nBid; bid++) {
    TRACE(NCCL_TCCL, "bid %d/%d -> channel %d", bid, nBid, least[bid]);
    int c = least[bid];
...
static void finishPlan(struct ncclKernelPlan* plan) {
  ...
  for (int c=0; c < MAXCHANNELS; c++) {
    ...
    if (tail != nullptr) {
      TRACE(NCCL_TCCL, "Channel %d has some work", c);
      channelUbound = c+1;
      channelCount += 1;
      channelMask |= 1ull<<c;
...
static ncclResult_t scheduleCollTasksToPlan(
  ...
  TRACE(NCCL_TCCL, "scheduleCollTasksToPlan done");
  return ncclSuccess;
}
...
ncclResult_t ncclLaunchKernel(struct ncclComm* comm, struct ncclKernelPlan* plan) {
  ...
  TRACE(NCCL_TCCL, "launchKernel %p grid %dx%dx%d block %dx%dx%d shmem %ld args %p/%p/%p", fn, grid.x, grid.y, grid.z, block.x, block.y, block.z, smem, args[0], args[1], args[2]);
...
static ncclResult_t getAlgoInfo(struct ncclInfo* info, int collNetTypeSupport, int numPipeOps) {
  ...
  if (comm->nRanks == 1) {
    ...
    info->algorithm = -1;
    info->protocol = -1;
    // First check whether we can use TCCL
    NCCLCHECK(ncclTopoGetTCCLAlgo(info));
    if (info->algorithm != NCCL_ALGO_TCCL) {
  ...
  if (info->protocol == NCCL_PROTO_SIMPLE) {
    ...
    if (info->algorithm == NCCL_ALGO_TCCL) nt += WARP_SIZE;
  ...
  info->nChannels = nc;
  info->nThreads = nt;
  TRACE(NCCL_TCCL, "nChannels %d nThreads %d", info->nChannels, info->nThreads);
  return ncclSuccess;
}
...
comp_next:
  ...
  *workFuncIndex = FUNC_INDEX(info->coll, info->opFull.op, info->datatype, info->algorithm, info->protocol);
  ...
  int chunkSteps = (info->protocol == NCCL_PROTO_SIMPLE && (info->algorithm == NCCL_ALGO_RING || info->algorithm == NCCL_ALGO_TCCL)) ? info->chunkSteps : 1;
  int sliceSteps = (info->protocol == NCCL_PROTO_SIMPLE && (info->algorithm == NCCL_ALGO_RING || info->algorithm == NCCL_ALGO_TCCL)) ? info->sliceSteps : 1;
  ...
  TRACE(NCCL_TCCL, "computeColl done");
  return ncclSuccess;
}
