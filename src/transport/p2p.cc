...
ncclResult_t p2pCanConnect(int* ret, struct ncclTopoSystem* topo, struct ncclTopoGraph* graph, struct ncclPeerInfo* info1, struct ncclPeerInfo* info2) {
  ...
  INFO(NCCL_TCCL, "p2pCanConnect between %lx and %lx : ncclTopoCheckP2p %d", info1->busId, info2->busId, *ret);
  ...
  INFO(NCCL_TCCL, "p2pCanConnect between %lx and %lx : ncclTopoCheckNet %d", info1->busId, info2->busId, useNet);
  if (useNet) {
    *ret = 0;
    return ncclSuccess;
  }
