...
ncclResult_t ncclShmOpen(char* shmPath, size_t shmSize, void** shmPtr, void** devShmPtr, int refcount, ncclShmHandle_t* handle, int numaIdx) {
  //INFO(NCCL_TCCL, "ncclShmOpen shmPath %s shmSize %ld refcount %d numaIdx %d", shmPath, shmSize, refcount, numaIdx);
  int fd = -1;
  char* hptr = NULL;
  void* dptr = NULL;
  ...
  *handle = *shmPtr = NULL; /* assume shmPtr and handle always set correctly by users. */
  ...
  // Handle TCCL's numa request
  if (create && numaIdx != -1) {
    NCCLCHECK(tcclSetNuma(hptr, realShmSize, numaIdx));
    INFO(NCCL_ALLOC | NCCL_TCCL, "Set %ld bytes of shared memory in %s to NUMA %d", realShmSize, shmPath, numaIdx);
  }
