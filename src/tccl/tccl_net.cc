...
#define NCCL_NET_MAP_HOSTMEM_NUMA 4 // NUMA-aware host memory used by TCCL; should be bank 4 to indicate !DEVMEM && !SHARED
#define NCCL_NET_MAP_GDCMEM 5
#define NCCL_NET_MAP_MEMS 6

// Let's use 3 bits for the bank index(originally 2 bits in NCCL), 1 bit for used, 28 bits for the offset
#define NCCL_NET_MAP_MASK_DEVMEM 0x20000000
#define NCCL_NET_MAP_MASK_SHARED 0x40000000
#define NCCL_NET_MAP_MASK_USED   0x10000000
#define NCCL_NET_MAP_MASK_OFFSET 0x0fffffff

#define NCCL_NET_MAP_OFFSET_BANK(mapStruct, offsetName) \
  ((mapStruct)->offsets.offsetName >> 29)

#define NCCL_NET_MAP_OFFSET_NULL(mapStruct, offsetName) \
  (((mapStruct)->offsets.offsetName >> 28) == 0)
...
// Use HOSTMEM_NUMA instead of NUMA
#define NCCL_NET_MAP_ADD_POINTER_NUMA(mapStruct, shared, dev, memSize, offsetName) do { \
    int bank = NCCL_NET_MAP_MASK_USED + (dev)*NCCL_NET_MAP_MASK_DEVMEM + (shared)*NCCL_NET_MAP_MASK_SHARED; \
    if ((shared) == 0) { \
      if (dev) { \
        (mapStruct)->offsets.offsetName = bank + (mapStruct)->mems[NCCL_NET_MAP_DEVMEM].size; \
        ...
        int bank = NCCL_NET_MAP_MASK_USED + (NCCL_NET_MAP_HOSTMEM_NUMA << 29); \
        (mapStruct)->offsets.offsetName = bank + (mapStruct)->mems[NCCL_NET_MAP_HOSTMEM_NUMA].size; \
        (mapStruct)->mems[NCCL_NET_MAP_HOSTMEM_NUMA].size += memSize; \
      } \
    } else { \
      (mapStruct)->offsets.offsetName = bank; \
    } \
...
struct connectMap {
  ...
  struct {
    ...
    uint32_t ceRecvMem;
    uint32_t ceBuffs[NCCL_NUM_PROTOCOLS];
  } offsets;
};
...
struct sendResources {
  ...
  char* ceBuffers[NCCL_NUM_PROTOCOLS];
...
static ncclResult_t tcclNetCanConnect(int* ret, struct ncclTopoSystem* topo, struct ncclTopoGraph* graph, struct ncclPeerInfo* info1, struct ncclPeerInfo* info2) {
  *ret = 1;
...
NCCL_PARAM(TcclNetSharedBuffers, "NET_SHARED_BUFFERS", -2);
NCCL_PARAM(TcclNetSharedComms, "NET_SHARED_COMMS", 1);
...
static ncclResult_t tcclNetSendSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* send, int channelId, int connIndex) {
  ...
  send->conn.shared = req.shared = graph ? 0 : ncclParamTcclNetSharedBuffers() != -2 ? ncclParamTcclNetSharedBuffers() : 1;
  if (req.shared) {
    WARN("TCCL_NET transport does not support shared buffer");
  ...
  // Instead of ncclTopoCheckGdr, use specified option
  req.useGdr = comm->tcclComm.sendOpt.net_send_use_gdr;
  send->conn.flags |= req.useGdr ? NCCL_DIRECT_NIC : 0;

  tpProxyRank = comm->topParentRanks[proxyRank];
  NCCLCHECK(ncclProxyConnect(comm, TRANSPORT_TCCL_NET, 1, tpProxyRank, &send->proxyConn));
  ...
  req.numaIdx = comm->tcclComm.sendOpt.net_numa_idx;
  req.useMemcpy = comm->tcclComm.sendOpt.net_use_memcpy;
  if (req.useGdr && req.useMemcpy) {
    WARN("TCCL_NET transport cannot use memcpy and GDRDMA at the same time");
    return ncclInternalError;
  }
...
NCCL_PARAM(TcclGdrCopySyncEnable, "GDRCOPY_SYNC_ENABLE", 1);
// GDRCOPY support: FLUSH_ENABLE When enabled uses a PCI-E read to flush GDRDMA buffers
NCCL_PARAM(TcclGdrCopyFlushEnable, "GDRCOPY_FLUSH_ENABLE", 0);

/* Setup recv connector */
static ncclResult_t tcclNetRecvSetup(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* recv, int channelId, int connIndex) {
  if (comm->proxyState->cudaCtx != NULL) {
    CUPFN(cuCtxSetCurrent(comm->proxyState->cudaCtx));
  }
  ...
  recv->conn.shared = req.shared = graph ? 0 : ncclParamTcclNetSharedBuffers() != -2 ? ncclParamTcclNetSharedBuffers() : 1;
  if (req.shared) {
    WARN("TCCL_NET transport does not support shared buffer");
    return ncclInternalError;
  }
  ...
  req.useGdr = comm->tcclComm.recvOpt.net_recv_use_gdr;
  ...
  NCCLCHECK(ncclProxyConnect(comm, TRANSPORT_TCCL_NET, 0, tpProxyRank, &recv->proxyConn));

  NCCLCHECK(ncclTopoGetLocalRank(comm->topo, myInfo->rank, &localRank));
  req.tpLocalRank = comm->topParentLocalRanks[localRank];
  req.tpRank = comm->topParentRanks[myInfo->rank];
  req.tpRemoteRank = comm->topParentRanks[peerInfo->rank];
  req.numaIdx = comm->tcclComm.recvOpt.net_numa_idx;
  req.useMemcpy = comm->tcclComm.recvOpt.net_use_memcpy;
  if (req.useGdr && req.useMemcpy) {
    WARN("TCCL_NET transport cannot use memcpy and GDRDMA at the same time");
    return ncclInternalError;
  }
...
static ncclResult_t netDumpMap(struct connectMap* map) {
  INFO(NCCL_TCCL, "Dump map same process %d shared %d", map->sameProcess, map->shared);
  struct connectMapMem *mem = map->mems+NCCL_NET_MAP_HOSTMEM;
  INFO(NCCL_TCCL, "Mem 0: Host mem %s (%x B) CPU %p GPU %p", mem->shmPath, mem->size, mem->cpuPtr, mem->gpuPtr);
  mem = map->mems+NCCL_NET_MAP_DEVMEM;
  INFO(NCCL_TCCL, "Mem 1: Vid  mem (%x B) CPU %p GPU %p", mem->size, mem->cpuPtr, mem->gpuPtr);
  mem = map->mems+NCCL_NET_MAP_SHARED_HOSTMEM;
  INFO(NCCL_TCCL, "Mem 2: Shared Host mem %s (%x B) CPU %p GPU %p", mem->shmPath, mem->size, mem->cpuPtr, mem->gpuPtr);
  mem = map->mems+NCCL_NET_MAP_SHARED_DEVMEM;
  INFO(NCCL_TCCL, "Mem 3: Shared Vid mem (%x B) CPU %p GPU %p", mem->size, mem->cpuPtr, mem->gpuPtr);
  mem = map->mems+NCCL_NET_MAP_HOSTMEM_NUMA;
  INFO(NCCL_TCCL, "Mem 4: Host numa mem (%x B) CPU %p GPU %p", mem->size, mem->cpuPtr, mem->gpuPtr);
  INFO(NCCL_TCCL, "SendMem -> Used %d Bank %d Offset %x, cpu %p gpu %p",
  ...
  INFO(NCCL_TCCL, "RecvMem -> Used %d Bank %d Offset %x, cpu %p gpu %p",
  ...
  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    INFO(NCCL_TCCL, "Proto %d -> Used %d Bank %d Offset %x, cpu %p, gpu %p", p,
  ...
  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    INFO(NCCL_TCCL, "CE Proto %d -> Used %d Bank %d Offset %x, cpu %p, gpu %p", p,
        map->offsets.ceBuffs[p] & NCCL_NET_MAP_MASK_USED ? 1 : 0,
        NCCL_NET_MAP_OFFSET_BANK(map, ceBuffs[p]), map->offsets.ceBuffs[p] & NCCL_NET_MAP_MASK_OFFSET,
        NCCL_NET_MAP_GET_POINTER(map, cpu, ceBuffs[p]), NCCL_NET_MAP_GET_POINTER(map, gpu, ceBuffs[p]));
  }
  INFO(NCCL_TCCL, "End of dump");
  return ncclSuccess;
}

static ncclResult_t tcclNetSendConnect(struct ncclComm* comm, struct ncclConnect* connectInfo, int nranks, int rank, struct ncclConnector* send) {
...
static ncclResult_t tcclNetRecvConnect(struct ncclComm* comm, struct ncclConnect* connectInfo, int nranks, int rank, struct ncclConnector* recv) {
...
static ncclResult_t tcclNetSendFree(struct ncclConnector* send) {
...
static ncclResult_t tcclNetRecvFree(struct ncclConnector* recv) {
...
static ncclResult_t sharedBuffersInit(struct ncclProxyState* proxyState, int cuda, int tpLocalRank, int type, int sameProcess,
    int nChannels, char** gpuPtr, char** cpuPtr, int* size, ncclIpcDesc *ipcDesc, int numaIdx = -1) {
  ...
  if (!cuda && state->hostBuff == NULL) {
    NCCLCHECK(ncclCudaHostCallocWithNuma(&state->hostBuff, state->size, numaIdx));
  }
...
static ncclResult_t tcclNetSendProxySetup(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  INFO(NCCL_TCCL, "tcclNetSendProxySetup entered");
  ...
  resources->numaIdx = req->numaIdx;
  resources->useMemcpy = req->useMemcpy;
...
static ncclResult_t tcclNetRecvProxySetup(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  INFO(NCCL_TCCL, "tcclNetRecvProxySetup entered");
  struct setupReq* req = (struct setupReq*) reqBuff;
  if (reqSize != sizeof(struct setupReq)) return ncclInternalError;
  ...
  INFO(NCCL_TCCL, "tcclNetRecvProxySetup before respSize check");
  if (respSize != sizeof(ncclNetHandle_t)) return ncclInternalError;
  INFO(NCCL_TCCL, "tcclNetRecvProxySetup before listen");
...
static ncclResult_t tcclNetSendProxyConnect(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  ...
  if (resources->shared) {
    ...
    if (resources->maxRecvs > 1 && ncclParamTcclNetSharedComms()) {
  ...
  INFO(NCCL_TCCL, "tcclNetSendProxyConnect connection->proxyAppendPtr set to %p", connection->proxyAppendPtr);
  *done = 1;
  ...
  if (resources->shared == 0) { // Only allocate dedicated buffers for ring/tree, not for p2p
    for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
      if (resources->useMemcpy) {
        // if useMemcpy, allocate both
        NCCL_NET_MAP_ADD_POINTER_NUMA(map, 0, 1, proxyState->buffSizes[p], buffs[p]);
        NCCL_NET_MAP_ADD_POINTER_NUMA(map, 0, 0, proxyState->buffSizes[p], ceBuffs[p]);
      } else if (p!= NCCL_PROTO_LL && resources->useGdr) {
        // Allocate GPU mem
        NCCL_NET_MAP_ADD_POINTER_NUMA(map, 0, 1, proxyState->buffSizes[p], buffs[p]);
      } else {
        // Allocate CPU mem
        NCCL_NET_MAP_ADD_POINTER_NUMA(map, 0, 0, proxyState->buffSizes[p], buffs[p]);
      }
    ...
    NCCLCHECK(sharedBuffersInit(
          ...
          &mapMem->gpuPtr, &mapMem->cpuPtr, &mapMem->size, &mapMem->ipcDesc, resources->numaIdx));
  ...
  NCCL_NET_MAP_ADD_POINTER(map, 0, 0, sizeof(struct ncclRecvMem), ceRecvMem);
  ...
  // Allocate NCCL_NET_MAP_HOSTMEM_NUMA bank
  if (map->mems[NCCL_NET_MAP_HOSTMEM_NUMA].size) {
    NCCLCHECK(ncclCudaHostCallocWithNuma(&map->mems[NCCL_NET_MAP_HOSTMEM_NUMA].cpuPtr, map->mems[NCCL_NET_MAP_HOSTMEM_NUMA].size, resources->numaIdx));
    map->mems[NCCL_NET_MAP_HOSTMEM_NUMA].gpuPtr = map->mems[NCCL_NET_MAP_HOSTMEM_NUMA].cpuPtr;
  }

  if (ncclGdrCopy && map->sameProcess && ncclParamTcclGdrCopySyncEnable()) {
  ...
  resources->ceRecvMem = (struct ncclRecvMem*) NCCL_NET_MAP_GET_POINTER(map, cpu, ceRecvMem);

  // Don't give credits yet in shared mode.
  resources->sendMem->head = map->shared ? -NCCL_STEPS : 0;
  for (int i=0; i<NCCL_STEPS; i++) {
    resources->recvMem->sizesFifo[i] = -1;
    resources->ceRecvMem->sizesFifo[i] = -1;
  }

  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    if (resources->useMemcpy) {
      // In case of memcpy, GPU --smwrite--> buffers --memcpy--> ceBuffer --isend--> NIC
      resources->buffers[p] = NCCL_NET_MAP_GET_POINTER(map, cpu, buffs[p]);
      resources->ceBuffers[p] = NCCL_NET_MAP_GET_POINTER(map, cpu, ceBuffs[p]);
    } else {
      resources->buffers[p] = resources->ceBuffers[p] = NCCL_NET_MAP_GET_POINTER(map, cpu, buffs[p]);
    }
    if (resources->ceBuffers[p]) { // register ceBuffer to NIC
#if CUDA_VERSION >= 11070
      /* DMA-BUF support */
      int type = NCCL_NET_MAP_DEV_MEM(map, buffs[p]) ? NCCL_PTR_CUDA : NCCL_PTR_HOST;
      if (type == NCCL_PTR_CUDA && resources->useDmaBuf) {
        int dmabuf_fd;
        CUCHECK(cuMemGetHandleForAddressRange((void *)&dmabuf_fd, (CUdeviceptr)resources->ceBuffers[p], resources->buffSizes[p], CU_MEM_RANGE_HANDLE_TYPE_DMA_BUF_FD, 0));
        NCCLCHECK(proxyState->ncclNet->regMrDmaBuf(resources->netSendComm, resources->ceBuffers[p], resources->buffSizes[p], type, 0ULL, dmabuf_fd, &resources->mhandles[p]));
        (void)close(dmabuf_fd);
      } else // FALL-THROUGH to nv_peermem GDR path
#endif
      {
        NCCLCHECK(proxyState->ncclNet->regMr(resources->netSendComm, resources->ceBuffers[p], resources->buffSizes[p], NCCL_NET_MAP_DEV_MEM(map, buffs[p]) ? NCCL_PTR_CUDA : NCCL_PTR_HOST, &resources->mhandles[p]));
      }
    }
  }
...
static ncclResult_t tcclNetRecvProxyConnect(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState, void* reqBuff, int reqSize, void* respBuff, int respSize, int* done) {
  if (proxyState->cudaCtx != NULL) {
    CUPFN(cuCtxSetCurrent(proxyState->cudaCtx));
  }
  ...
  INFO(NCCL_TCCL, "tcclNetRecvProxyConnect connection->proxyAppendPtr set to %p", connection->proxyAppendPtr);
  *done = 1;
  ...
  if (resources->shared == 0) { // Only allocate dedicated buffers for ring/tree, not for p2p
    ...
    NCCLCHECK(sharedBuffersInit(
          ...
          &mapMem->gpuPtr, &mapMem->cpuPtr, &mapMem->size, NULL, resources->numaIdx));
    resources->buffSizes[NCCL_PROTO_SIMPLE] = mapMem->size;
    NCCL_NET_MAP_ADD_POINTER(map, 1, resources->useGdr, mapMem->size, buffs[NCCL_PROTO_SIMPLE]);
  }
  ...
  if (ncclGdrCopy && map->sameProcess) {
    ...
    if (ncclParamTcclGdrCopySyncEnable()) {
      resources->gdcSync = cpuPtr;
      struct connectMapMem* gdcMem = map->mems+NCCL_NET_MAP_GDCMEM;
      gdcMem->cpuPtr = (char*)cpuPtr;
      gdcMem->gpuPtr = (char*)gpuPtr;
    ...
    if (ncclParamTcclGdrCopyFlushEnable()) resources->gdcFlush = cpuPtr + 1;
  }
  ...
  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    if (resources->useMemcpy) {
      // In case of memcpy, NIC --irecv--> buffers --memcpy--> ceBuffers --smread--> GPU
      resources->buffers[p] = NCCL_NET_MAP_GET_POINTER(map, cpu, buffs[p]);
      resources->ceBuffers[p] = NCCL_NET_MAP_GET_POINTER(map, cpu, ceBuffs[p]);
    } else {
      resources->buffers[p] = resources->ceBuffers[p] = NCCL_NET_MAP_GET_POINTER(map, cpu, buffs[p]);
    ...
    if (resources->ceBuffers[p]) { // register ceBuffer to NIC
      ...
      if (type == NCCL_PTR_CUDA && resources->useDmaBuf) {
        int dmabuf_fd;
        CUCHECK(cuMemGetHandleForAddressRange((void *)&dmabuf_fd, (CUdeviceptr)resources->ceBuffers[p], resources->buffSizes[p], CU_MEM_RANGE_HANDLE_TYPE_DMA_BUF_FD, 0));
        NCCLCHECK(proxyState->ncclNet->regMrDmaBuf(resources->netRecvComm, resources->ceBuffers[p], resources->buffSizes[p], type, 0ULL, dmabuf_fd, &resources->mhandles[p]));
        (void)close(dmabuf_fd);
      } else // FALL-THROUGH to nv_peermem GDR path
#endif
      {
        NCCLCHECK(proxyState->ncclNet->regMr(resources->netRecvComm, resources->ceBuffers[p], resources->buffSizes[p], NCCL_NET_MAP_DEV_MEM(map, buffs[p]) ? NCCL_PTR_CUDA : NCCL_PTR_HOST, &resources->mhandles[p]));
      }
    }
  }
...
static ncclResult_t tcclNetSendProxyFree(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState) {
  struct sendResources* resources = (struct sendResources*)(connection->transportResources);
  ...
  if (connection->state == connConnected) {
    ...
    if (mems[NCCL_NET_MAP_HOSTMEM_NUMA].cpuPtr) NCCLCHECK(ncclCudaHostFree(mems[NCCL_NET_MAP_HOSTMEM_NUMA].cpuPtr));
    if (resources->shared) {
...
static ncclResult_t tcclNetRecvProxyFree(struct ncclProxyConnection* connection, struct ncclProxyState* proxyState) {
  struct recvResources* resources = (struct recvResources*)(connection->transportResources);
  if (connection->state == connSharedInitialized) { // NVB Preconnect
...
static ncclResult_t tcclNetSendProxyProgress(struct ncclProxyState* proxyState, struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpReady) {
    for (int s=0; s<args->nsubs; s++) {
      ...
      // We use "memcpyed" to indicate memcpy'ed but not yet transmitted. (transmitted < memcpyed < posted)
      sub->memcpyed = 0;
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s=0; s<args->nsubs; s++) {
      ...
      if (resources->useMemcpy) {
        if (sub->memcpyed < sub->posted && sub->memcpyed < sub->transmitted + NCCL_STEPS) {
          int buffSlot = (sub->base+sub->memcpyed)%NCCL_STEPS;
          volatile int* sizesFifo = resources->recvMem->sizesFifo;
          volatile uint64_t* recvTail = &resources->recvMem->tail;
          //if (*recvTail >= sub->base + sub->memcpyed + NCCL_STEPS && !warn_once) {
          //  INFO(NCCL_TCCL, "TCCL_NET send GPU buffer full!!!");
          //  warn_once = 1;
          //}
          if (p != NCCL_PROTO_SIMPLE) {
            WARN("TCCL_NET Memcpy is only supported for simple protocol");
            return ncclInternalError;
          }
          if (sizesFifo[buffSlot] != -1 && (*recvTail > (sub->base+sub->memcpyed))) {
            // We have something to receive, let's check if it's completely ready.
            int size = sizesFifo[buffSlot];
            char* buff = localBuff+buffSlot*stepSize;
            char* celocalBuff = NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, ceBuffs[p]);
            char* ceBuff = celocalBuff+buffSlot*stepSize;
            CUDACHECK(cudaMemcpyAsync(ceBuff, buff, size, cudaMemcpyDeviceToHost, resources->stream));
            CUDACHECK(cudaEventRecord(resources->events[buffSlot], resources->stream));
            resources->ceRecvMem->sizesFifo[buffSlot] = size;
            __sync_synchronize(); // make sure sizesFifo is visible
            sub->memcpyed += args->sliceSteps;
            sizesFifo[buffSlot] = -1;
            args->idle = 0;
            continue;
          }
        }
        if (sub->transmitted < sub->memcpyed && sub->transmitted < sub->done + NCCL_STEPS) {
          //if (sub->memcpyed >= sub->transmitted + NCCL_STEPS && !warn_once) {
          //  INFO(NCCL_TCCL, "TCCL_NET send CPU buffer full!!!");
          //  warn_once = 1;
          //}
          ...
          if (res == cudaSuccess) {
            ...
            int size = resources->ceRecvMem->sizesFifo[buffSlot];
            char* celocalBuff = NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, ceBuffs[p]);
            char* ceBuff = celocalBuff+buffSlot*stepSize;
            NCCLCHECK(proxyState->ncclNet->isend(resources->netSendComm, ceBuff, size, resources->tpRank, mhandle, sub->requests+buffSlot));
            if (sub->requests[buffSlot] != NULL) {
              TRACE(NCCL_TCCL, "sendProxy [%ld/%d] Isend posted, req %p", sub->transmitted, buffSlot, sub->requests[buffSlot]);
              sub->transmitted += args->sliceSteps;
              // Notify SHM
              resources->ceRecvMem->tail = sub->base + sub->transmitted;
      ...
      if (sub->done < sub->transmitted) {
        ...
        TRACE(NCCL_TCCL, "sendProxy [%ld/%d] testing req %p", sub->done, buffSlot, sub->requests[buffSlot]);
...
static ncclResult_t tcclNetRecvProxyProgress(struct ncclProxyState* proxyState, struct ncclProxyArgs* args) {
  //static int warn_once = 0;
  if (args->state == ncclProxyOpReady) {
    ...
    for (int s=0; s<args->nsubs; s++) {
      ...
      // We use "memcpyed" to indicate memcpy'ed but not yet transmitted. (transmitted < memcpyed < received)
      sub->memcpyed = 0;
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    // irecv step (sub->posted); use ceBuffs if useMemcpy
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      ...
      for (int i=0; i<subGroup->groupSize; i++) {
        ...
        if (sub->posted < sub->nsteps) {
          ...
          char* localBuff = NULL;
          if (resources->useMemcpy) {
            localBuff = NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, ceBuffs[p]);
          } else {
            localBuff = NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, buffs[p]);
          }
          int buffSlot = (sub->base+sub->posted)%NCCL_STEPS;
    ...
    // test & flush step (sub->received); use ceBuffs if useMemcpy
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      struct ncclProxySubArgs* subGroup = args->subs+s;
      struct recvResources* resources = (struct recvResources*) (subGroup->connection->transportResources);
      if (subGroup->posted > subGroup->received) {
        ...
        if (done) {
          ...
          for (int i=0; i<subGroup->groupSize; i++) {
            ...
            resources->ceRecvMem->sizesFifo[buffSlot] = sizes[i];
    ...
    // memcpy step (sub->memcpyed); only if useMemcpy
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      struct ncclProxySubArgs* subGroup = args->subs+s;
      struct recvResources* resources = (struct recvResources*) (subGroup->connection->transportResources);
      if (!resources->useMemcpy) continue;
      if (subGroup->received > subGroup->memcpyed) {
        //if (subGroup->received >= subGroup->memcpyed + NCCL_STEPS && !warn_once) {
        //  INFO(NCCL_TCCL, "TCCL_NET recv CPU buffer full!!!");
        //  warn_once = 1;
        //}
        uint64_t step = subGroup->memcpyed;
        int done = 1;
        ...
        if (done) {
          for (int i=0; i<subGroup->groupSize; i++) {
            ...
            if (step < sub->nsteps) {
              ...
              char* celocalBuff = NCCL_NET_MAP_GET_POINTER(&resources->map, cpu, ceBuffs[p]);
              char* ceBuff = celocalBuff+buffSlot*stepSize;
              size_t count = resources->ceRecvMem->sizesFifo[buffSlot];
              CUDACHECK(cudaMemcpyAsync(buff, ceBuff, count, cudaMemcpyHostToDevice, resources->stream));
              CUDACHECK(cudaEventRecord(resources->events[buffSlot], resources->stream));
              sub->memcpyed += args->sliceSteps;
            }
          }
    ...
    // transmit (after memcpy) step (sub->transmitted); only if useMemcpy
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      struct ncclProxySubArgs* subGroup = args->subs+s;
      struct recvResources* resources = (struct recvResources*) (subGroup->connection->transportResources);
      if (!resources->useMemcpy) continue;
      if (subGroup->memcpyed > subGroup->transmitted) {
        //if (subGroup->memcpyed >= subGroup->transmitted + NCCL_STEPS && !warn_once) {
        //  INFO(NCCL_TCCL, "TCCL_NET recv GPU buffer full!!!");
        //  warn_once = 1;
        //}
        uint64_t step = subGroup->transmitted;
        int buffSlot = (subGroup->base+subGroup->transmitted)%NCCL_STEPS;
        cudaError_t res = cudaEventQuery(resources->events[buffSlot]);
        if (res != cudaErrorNotReady) CUDACHECK(res);
        if (res == cudaSuccess) {
          for (int i=0; i<subGroup->groupSize; i++) {
    ...
    // transmit(i.e., gpu.recvTail++) step (sub->transmitted); only if !useMemcpy
    for (int s=0; s<args->nsubs; s+=args->subs[s].groupSize) {
      struct ncclProxySubArgs* subGroup = args->subs+s;
      struct recvResources* resources = (struct recvResources*) (subGroup->connection->transportResources);
      if (resources->useMemcpy) continue;
...
struct ncclTransport tcclNetTransport = {
  "TCCL_NET",
  tcclNetCanConnect,
  { tcclNetSendSetup, tcclNetSendConnect, tcclNetSendFree, proxySharedInit,
    tcclNetSendProxySetup, tcclNetSendProxyConnect, tcclNetSendProxyFree, tcclNetSendProxyProgress },
  { tcclNetRecvSetup, tcclNetRecvConnect, tcclNetRecvFree, proxySharedInit,
    tcclNetRecvProxySetup, tcclNetRecvProxyConnect, tcclNetRecvProxyFree, tcclNetRecvProxyProgress }
};
