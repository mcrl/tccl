...
static ncclResult_t SaveProxy(struct ncclComm* comm, struct ncclChannel* channel, int type, int peer, struct ncclProxyOp* op, int connIndex, bool* justInquire) {
  ...
  TRACE(NCCL_TCCL, "channel->id=%d channel->peers=%p peer=%d", channel->id, channel->peers, peer);
  ...
  // TCCL transports will have proxyProgress even if they don't need a proxy
  // So we should check its internal flags
  if (connector->transportComm->proxyProgress == tcclP2pTransport.send.proxyProgress) {
    int needProgress = *(int*)((char*)connector->transportResources + tcclP2pTransportSendResourceMemcpyOffset);
    TRACE(NCCL_TCCL, "P2p send case) needProgress=%d", needProgress);
    if (!needProgress) return ncclSuccess;
  }
  if (connector->transportComm->proxyProgress == tcclShmTransport.send.proxyProgress) {
    int needProgress = *(int*)((char*)connector->transportResources + tcclShmTransportSendResourceMemcpyOffset);
    TRACE(NCCL_TCCL, "Shm send case) needProgress=%d", needProgress);
    if (!needProgress) return ncclSuccess;
  }
  if (connector->transportComm->proxyProgress == tcclShmTransport.recv.proxyProgress) {
    int needProgress = *(int*)((char*)connector->transportResources + tcclShmTransportRecvResourceMemcpyOffset);
    TRACE(NCCL_TCCL, "Shm recv case) needProgress=%d", needProgress);
    if (!needProgress) return ncclSuccess;
  }
...
void* ncclProxyProgress(void *proxyState_) {
  ...
  // TODO
  {
    ...
    INFO(NCCL_TCCL, "Proxy thread context before creating new %p", ctx);
  }
  ...
  // TODO
  {
    CUcontext ctx;
    CUPFN(cuCtxGetCurrent(&ctx));
    INFO(NCCL_TCCL, "Proxy thread context after creating new %p", ctx);
  }
...
static ncclResult_t proxyProgressAsync(struct ncclProxyAsyncOp* op, struct ncclProxyState* proxyState, int* asyncOpCount, struct ncclProxyLocalPeer* peer, struct ncclProxyConnectionPool* connectionPool) {
  ...
  if (op->type == ncclProxyMsgSetup) {
    ...
    //INFO(NCCL_TCCL, "proxyProgressAsync::proxySetup() opId=%p proxySetup func=%p", op->opId, op->connection->tcomm->proxySetup);
    //INFO(NCCL_TCCL, "TCCL_NET sendproxysetup=%p recvproxysetup=%p", tcclNetTransport.send.proxySetup, tcclNetTransport.recv.proxySetup);
    //INFO(NCCL_TCCL, "%p %p %p %p %p %p %p %p %p %p", tcclShmTransport.send.proxySetup, tcclShmTransport.recv.proxySetup,
    //    tcclP2pTransport.send.proxySetup, tcclP2pTransport.recv.proxySetup,
    //    netTransport.send.proxySetup, netTransport.recv.proxySetup,
    //    shmTransport.send.proxySetup, shmTransport.recv.proxySetup,
    //    p2pTransport.send.proxySetup, p2pTransport.recv.proxySetup);
