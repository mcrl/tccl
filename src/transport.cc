...
static ncclResult_t selectTransport(struct ncclComm* comm, struct ncclTopoGraph* graph, struct ncclConnect* connect, int channelId, int peer, int connIndex, int* transportType) {
  ...
  if (comm->tcclComm.enabled) {
    bool isSend = type == 1;
    comm->tcclComm.tmpIsSend = isSend;
    struct ncclTransport *transport = NULL;
    int tcclTransportType = isSend ? comm->tcclComm.sendOpt.type : comm->tcclComm.recvOpt.type;
    int ncclTransportType = -1;
    if (tcclTransportType == TCCL_TRANSPORT_TYPE_P2P) {
      transport = &tcclP2pTransport;
      ncclTransportType = TRANSPORT_P2P; 
    } else if (tcclTransportType == TCCL_TRANSPORT_TYPE_SHM) {
      transport = &tcclShmTransport;
      ncclTransportType = TRANSPORT_SHM; 
    } else if (tcclTransportType == TCCL_TRANSPORT_TYPE_NET) {
      transport = &tcclNetTransport;
      ncclTransportType = TRANSPORT_NET; 
    } else {
      WARN("tcclTransportType %d is not supported yet", tcclTransportType);
      return ncclSystemError;
    }
    struct ncclTransportComm* transportComm = isSend ? &transport->send : &transport->recv;
    int ret = 0;
    INFO(NCCL_TCCL, "Trying transport: rank %d[%lx] -> rank %d[%lx] via %s", myInfo->rank, myInfo->busId, peerInfo->rank, peerInfo->busId, transport->name);
    NCCLCHECK(transport->canConnect(&ret, comm->topo, graph, myInfo, peerInfo));
    INFO(NCCL_TCCL, "Trying transport: rank %d[%lx] -> rank %d[%lx] via %s ret=%d", myInfo->rank, myInfo->busId, peerInfo->rank, peerInfo->busId, transport->name, ret);
    if (ret) {
      INFO(NCCL_TCCL, "Connected: rank %d[%lx] -> rank %d[%lx] via %s", myInfo->rank, myInfo->busId, peerInfo->rank, peerInfo->busId, transport->name);
      connector->transportComm = transportComm;
      NCCLCHECK(transportComm->setup(comm, graph, myInfo, peerInfo, connect, connector, channelId, connIndex));
      if (transportType) *transportType = ncclTransportType;
      return ncclSuccess;
    }
  } else {
    // Search only up to collnet (excluding tccl transports)
    for (int t = 0; t <= TRANSPORT_COLLNET; t++) {
      struct ncclTransport *transport = ncclTransports[t];
      struct ncclTransportComm* transportComm = type == 1 ? &transport->send : &transport->recv;
      int ret = 0;
      NCCLCHECK(transport->canConnect(&ret, comm->topo, graph, myInfo, peerInfo));
      if (ret) {
        TRACE(NCCL_TCCL, "rank %d[%lx] -> rank %d[%lx] via %s", myInfo->rank, myInfo->busId, peerInfo->rank, peerInfo->busId, transport->name);
        connector->transportComm = transportComm;
        NCCLCHECK(transportComm->setup(comm, graph, myInfo, peerInfo, connect, connector, channelId, connIndex));
...
ncclResult_t ncclTcclConnectSend(struct ncclComm* comm, int channelId, int nsend, int* peerSend, int connIndex) {
  TRACE(NCCL_INIT, "nsend %d nrecv %d", nsend, nrecv);
  struct ncclChannel* channel = &comm->channels[channelId];
  uint64_t mask = 1UL << channel->id;
  for (int i=0; i<nsend; i++) {
