...
const char* ncclAlgoStr[NCCL_NUM_ALGORITHMS] = { "Tree", "Ring", "CollNetDirect", "CollNetChain", "NVLS", "NVLSTree", "TCCL" };
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent = NULL) {
  ...
  // Assign a dummy ringGraph for TCCL, so the codes that enumerate NCCL_NUM_ALGORITHMS do not fail
  struct ncclTopoGraph* graphs[] = { &treeGraph, &ringGraph, &collNetGraph, &collNetGraph, &nvlsGraph, &nvlsGraph, &ringGraph };
  ...
  if (comm->tcclComm.enabled) {
    NCCLCHECKGOTO(tcclInit(comm, &ringGraph), ret, fail);
  } else {
    INFO(NCCL_TCCL, "TCCL is disabled. (nRanks=%d)", comm->nRanks);
  }
