...
ncclResult_t ncclTopoTuneModel(struct ncclComm* comm, int minCompCap, int maxCompCap, struct ncclTopoGraph** graphs) {
  ...
  comm->maxThreads[NCCL_ALGO_TCCL][NCCL_PROTO_SIMPLE] =
    getNthreads("NCCL_NTHREADS", ncclParamNthreads(), 2*WARP_SIZE, NCCL_SIMPLE_MAX_NTHREADS, simpleDefaultThreads);
  ...
  for (int coll=0; coll<NCCL_NUM_FUNCTIONS; coll++) {
    ...
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      if (coll == ncclFuncBroadcast && a != NCCL_ALGO_RING && a != NCCL_ALGO_TCCL) continue;
      if (coll == ncclFuncReduce && a != NCCL_ALGO_RING && a != NCCL_ALGO_TCCL) continue;
      if (coll == ncclFuncReduceScatter && a != NCCL_ALGO_RING && a != NCCL_ALGO_TCCL) continue;
      if (coll == ncclFuncAllGather && a != NCCL_ALGO_RING && a != NCCL_ALGO_TCCL) continue;

      for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
        // Mark TCCL bandwidth with non-zero value; it will be set to zero later if disabled
        if (a == NCCL_ALGO_TCCL) {
  ...
  // TCCL disabled by default
  int algoEnable[NCCL_NUM_ALGORITHMS] = { 1, 1, 1, 1, 1, 1, 0 };
  ...
  if (algoEnable[NCCL_ALGO_TCCL]) comm->tcclComm.enabled = true;
  ...
  for (int c=0; c<NCCL_NUM_FUNCTIONS; c++) for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    ...
    //WARN("c %d a %d p %d bandwidth %f", c, a, p, comm->bandwidths[c][a][p]);
  }
...
ncclResult_t ncclTopoGetTCCLAlgo(struct ncclInfo* info) {
  if (info->comm->tcclComm.enabled) {
    info->algorithm = NCCL_ALGO_TCCL;
